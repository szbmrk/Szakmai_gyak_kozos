...
app.get("/teachers", (req, res) => {
  db.query(
    "SELECT teacher.*, GROUP_CONCAT(Course.course_name) AS courses FROM teacher LEFT JOIN course ON teacher.teacher_id = course.teacher_id GROUP BY teacher.teacher_id;",
    (err, results) => {
      if (err) {
...
app.get("/courses/:student_id", (req, res) => {
  ...
  db.query(
    `SELECT * FROM Course LEFT JOIN Enrollment ON Enrollment.course_id = Course.course_id 
...
app.get("/courses/:course_id", (req, res) => {
  ...
  db.query(
    `SELECT * FROM course_contents WHERE course_id = ${course_id}`,
    (err, results) => {
      if (err) {
...
app.get("/assignment/:teacher_id", (req, res) => {
  ...
  db.query(
    `SELECT assignments.*, course.course_name FROM assignments JOIN course ON assignments.course_id = course.course_id JOIN teacher ON course.teacher_id = teacher.teacher_id WHERE teacher.teacher_id = ${teacher_id};`,
    (err, results) => {
      if (err) {
        console.error("Error retrieving assignments:", err);
        res.status(500).json({ error: "Failed to retrieve assignments" });
...
app.get("/available-courses/:student_id", (req, res) => {
  const student_id = req.params.student_id;
  db.query(
    `SELECT * FROM Course WHERE course_id NOT IN (SELECT course_id FROM Enrollment WHERE student_id = ${student_id})`,
    (err, results) => {
      if (err) {
...
app.post("/enroll", (req, res) => {
  ...
  db.query(
    `INSERT INTO Enrollment (student_id, course_id) VALUES (${student_id}, ${course_id})`,
    (err, results) => {
      if (err) {
...
app.post("/student-login", (req, res) => {
  ...
  db.query(
    `SELECT * FROM Student WHERE student_email = '${email}' AND student_password = '${password}'`,
    (err, results) => {
      if (err) {
...
app.post("/teacher-login", (req, res) => {
  const { email, password } = req.body;
  db.query(
    `SELECT * FROM Teacher WHERE teacher_email = '${email}' AND teacher_password = '${password}'`,
    (err, results) => {
      if (err) {
        console.error("Error retrieving student:", err);
        res.status(500).json({ error: "Failed to retrieve student" });
...
app.post("/assignments/:course_id", (req, res) => {
  ...
  const course_id = req.params.course_id;
  db.query(
    `INSERT INTO Assignments (course_id, title, description) VALUES (${course_id}, '${assignment_name}', '${assignment_description}')`,
    (err, results) => {
      if (err) {
...
app.post("/courses", (req, res) => {
  ...
  db.query(
    `INSERT INTO Course (teacher_id, course_name, course_description) VALUES (${teacher_id}, '${course_name}', '${course_description}')`,
    (err, results) => {
      if (err) {
...
app.put("/assignments/:assignment_id", (req, res) => {
  ...
  const { assignment_name, assignment_description } = req.body;
  db.query(
    `UPDATE Assignments SET title = '${assignment_name}', description = '${assignment_description}' WHERE assignment_id = ${assignment_id}`,
    (err, results) => {
      if (err) {
...
app.put("/courses/:course_id", (req, res) => {
  ...
  db.query(
    `UPDATE Course SET course_name = '${course_name}', course_description = '${course_description}' WHERE course_id = ${course_id}`,
    (err, results) => {
      if (err) {
...
app.get("/assignment_state", (req, res) => {
...
app.get("/assignments/:courseId/:studentId", (req, res) => {
